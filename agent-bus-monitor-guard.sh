...
printf '%s' '{"decision":"block","reason":"No agent-bus monitor is armed. Run: agent-bus-cli.sh wake with the Bash tool (run_in_background) -- not the Monitor tool, which the harness caps at 30 minutes -- then you may stop. Do not pass --ack -- once you have handled a message, ack its id (agent-bus-cli.sh ack <id>...), never on delivery and never ack-all.","hookSpecificOutput":{"hookEventName":"Stop","additionalContext":"No agent-bus monitor is armed. Run agent-bus-cli.sh wake with the Bash tool (run_in_background), never the Monitor tool, which the harness caps at 30 minutes; re-arm before stopping. Do not pass --ack: it drains on delivery, so a session that dies mid-handle destroys the DM. Ack means handled -- run agent-bus-cli.sh ack <id>... for the messages you acted on; ack-all would also remove mail that arrived unread."}}'
exit 0
