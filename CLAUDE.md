...
Both tools are preinstalled in the dev container (`dev-container.sh`'s embedded default image). To manually test `dev-container.sh`, run it against a scratch folder (`DEV_CONTAINER_HOME=/tmp/dc dev! test /some/folder`) — it requires Docker, auto-writes a fallback Dockerfile when none is present, and prompts interactively before creating/killing anything.
...
- **Dockerfile resolution**: `$folder/Dockerfile.dev` (context `$folder`) → else `$STATE_HOME/Dockerfile` (context state home). If neither exists, `write_default_dockerfile` writes a batteries-included default (Alpine + bash/git/curl, Claude Code, chezmoi dotfiles, tmux, and the creds-injecting entrypoint) to `$STATE_HOME/Dockerfile` on first use — the script is distributed standalone, so this default is **embedded as a heredoc** rather than read from disk — it is the single source for this repo's own dev container too (there is no committed `Dockerfile.dev`; edit the heredoc to change it). `dev! init` regenerates it: no base image → the full default; `init <base-image>` → a minimal skeleton `FROM <base-image>`; `--global` targets `$STATE_HOME/Dockerfile` instead of `./Dockerfile.dev`. Each `Dockerfile.dev`'s only hard requirement is a `dev` user with bash, created with `HOST_UID`/`HOST_GID` build args for volume permission parity.
